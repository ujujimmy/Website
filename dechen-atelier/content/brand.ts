...
export const brand = {
   ...
   *
   * TODO(salon): the shop number and street for each.
   *
   * What's here is the neighbourhood — accurate, but not enough to walk to.
   * Neither could be verified from the build environment (dechensalon.com does
   * not resolve, and no directory listing carries the numbers), and inventing
   * one would send clients to the wrong door. The directions links search Maps
   * for each branch by name until real addresses arrive, which lands people in
   * the right place but is a search rather than a pin.
   */
  locations: [
    {
      id: "salon",
      name: "DECHEN Salon",
      /** What this branch is, in three or four words. */
      role: "The salon",
      ...
      /**
       * Google's official directions deep link. On a phone it opens the Maps
       * app with navigation already aimed at the branch, which is the only
       * version of "directions" anyone actually wants.
       */
      directionsUrl:
 ...
 * metadata, the JSON-LD anchor, the footer's single-line address.
 */
